...
function EventBadges({ events, t }: { events: Event[]; t: ReturnType<typeof useTranslation>['t'] }) {
  ...
  const getBeverageLabel = (beverage: 'water' | 'protein' | 'coffee' | 'tea' | 'other') => {
    switch (beverage) {
      case 'water':
        return t('notes.beverageWater');
      case 'protein':
        return t('notes.beverageProtein');
      case 'coffee':
        return t('notes.beverageCoffee');
      case 'tea':
        return t('notes.beverageTea');
      default:
        return t('notes.beverageOther');
    }
  };

  const getWorkoutLabel = (sport: string) => {
    switch (sport) {
      case 'hiit_hyrox':
        return t('notes.workoutHiit');
      case 'cardio':
        return t('notes.workoutCardio');
      case 'gym':
        return t('notes.workoutGym');
      case 'swimming':
        return t('notes.workoutSwimming');
      case 'football':
        return t('notes.workoutFootball');
      default:
        return sport;
    }
  };
          ...
          case 'drink':
            return (
              <span key={event.id} className={baseClass}>
                ...
                {confidenceLow && <span className="ml-2 text-xs">⚠︎ {t('notes.confidenceLow')}</span>}
              </span>
            );
          case 'protein':
            return (
...
function NoteCard({ note, t }: { note: SmartNote; t: ReturnType<typeof useTranslation>['t'] }) {
  ...
  const handleDelete = useCallback(async () => {
    const confirmed = typeof window === 'undefined' ? true : window.confirm(t('notes.deleteConfirm'));
    if (!confirmed) return;
    setIsDeleting(true);
    try {
  ...
  return (
    <div className={`${glassCardHoverClasses} ${designTokens.padding.compact} text-white space-y-3 w-full`}>
      ...
      <EventBadges events={note.events} t={t} />
