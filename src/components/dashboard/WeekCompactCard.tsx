    ...
    <div className="rounded-2xl bg-white/5 dark:bg-white/5 backdrop-blur-md border border-white/10 shadow-[0_6px_24px_rgba(0,0,0,0.25)] p-4 lg:p-5 transition-all duration-200">
      ...
      <div className="flex items-start justify-between mb-3 gap-4">
        ...
        <div className="text-xs text-white/70 whitespace-nowrap flex-shrink-0">
          Mind. {minTasks} Aufgaben ({totalTasksDone}/35 {t('dashboard.tasks')})
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 lg:gap-2.5">
        {weekDays.map((day) => {
          let chipClasses = 'inline-flex items-center justify-center h-9 min-w-[2.75rem] px-3 rounded-full border text-sm font-medium transition-all cursor-pointer select-none';
